...
# Default: single long window (configurable via CLI). If WINDOW_START_S is None,
# the legacy short WINDOWS list (10ms) is used.
WINDOW_START_S = 5.000  # Start time (s) for long window
WINDOW_DURATION_S = 5.0  # Duration (s) for long window

# Legacy short windows (used only if WINDOW_START_S is None)
WINDOWS = [
    (5.000, 5.010),
    (8.200, 8.210),
    (9.000, 9.010),
...
# DT values to test (in milliseconds) - FINER RESOLUTION for Angus's analysis
# Primary zoom (0-2ms): 0.1ms steps
# Context tail (2-3ms): 0.25ms steps
DT_RANGE_MS = (0, 3)  # Focus on 0-3ms range
DT_STEP_MS = 0.1      # 0.1ms steps for fine resolution

# Output directory (using mounted Windows partition path as requested)
OUTPUT_DIR = "/media/sumit/New Volume1/window_predictions_5s_fine"
...
def process_window_predictions(output_dir: str, window_start_s: float = None, window_duration_s: float = None,
    ...
    # Generate dt values with different resolutions as Angus suggested
    # Primary zoom (0-2ms): 0.1ms steps
    # Context tail (2-3ms): 0.25ms steps
    ...
    print(f"Testing {len(dt_values_ms)} dt values:")
    ...
    for window_idx, window in enumerate(candidate_windows):
        ...
        for dt_ms in dt_values_ms:
            ...
            print(f"  Processing dt={dt_ms:4.1f}ms ({current_combination:2d}/{total_combinations})", end=" ... ")
            ...
            pred_fname = os.path.join(window_dir, f"pred_events_dt_{dt_ms:04.1f}ms." + ("npz" if use_compressed else "npy"))
            if use_compressed:
            ...
            if save_combined:
                ...
                comb_fname = os.path.join(window_dir, f"combined_events_dt_{dt_ms:04.1f}ms." + ("npz" if use_compressed else "npy"))
                if use_compressed:
...
def main():
    ...
    use_legacy = args.window_start is not None and args.window_start < 0
    ...
    if use_legacy:
        ...
        ws = args.window_start
        wd = args.window_duration
        print(f"Using single long window: start {ws:.3f}s, duration {wd:.3f}s")

    print(f"DT range: 0-3ms (0.1ms steps for 0-2ms, 0.25ms steps for 2-3ms)")
