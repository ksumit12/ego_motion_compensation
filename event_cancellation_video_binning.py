#!/usr/bin/env python3
"""
Memory-Efficient Event Cancellation and Video Generation
======================================================

This script uses time binning for BOTH cancellation and video generation:
1. Divides events into time bins (5ms bins)
2. Performs cancellation within each bin (faster, memory-safe)
...
def cancel_in_bin(real_bin: np.ndarray, pred_bin: np.ndarray, r_pix: float, 
    ...
    """
    TIME BINNING CANCELLATION: Cancel events within a single time bin.
    
    This is much faster and more memory-efficient than processing all events at once.
    Events are already grouped by time bin, so we only need to match within each bin.
    """
...
def render_video_with_binning(combined_events: np.ndarray, dt_estimate: float):
    """
    Render video using time binning approach - much more memory efficient.
    
    This approach:
    1. Divides timeline into bins (BIN_MS width)
    2. Performs cancellation within each bin
    3. Creates video frame from residual events in each bin
    """
    t = combined_events[:, 3]
    tmin = float(t.min())
    tmax = float(t.max())
    ...
    print(f"Time binning approach: cancellation + video generation both use {BIN_MS}ms bins")
    ...
    for b in range(len(edges) - 1):
        ...
        if i1 <= i0:
            ...
            real_bin = bin_events[bin_events[:, 4] == 0.0][:, :4]  # Keep x,y,polarity,timestamp
            pred_bin = bin_events[bin_events[:, 4] == 1.0][:, :4]  # Keep x,y,polarity,timestamp
            ...
            residual_real = real_bin[unmatched_real_mask][:, :3]  # Only x,y,polarity for image
            residual_pred = pred_bin[unmatched_predicted_mask][:, :3]  # Only x,y,polarity for image
...
if __name__ == "__main__":
    print("Memory-Efficient Event Cancellation and Video Generation")
    print("=" * 60)
    print(f"Parameters:")
